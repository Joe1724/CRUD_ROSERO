...
use App\Models\PostModel;  // Correct use statement for PostModel
use Config\App;
...
class PostController extends BaseController {
    ...
    public function add() {
        ...
        if (!$validation->withRequest($this->request)->run()) {
            ...
            $postModel = new PostModel();  // Use PostModel directly
            $postModel->save($data);
            return $this->response->setJSON([
                'error' => false,
    ...
    public function fetch() {
        ...
        if ($posts) {
            ...
            return $this->response->setJSON([
                'error' => false,
                'message' => '<div class="text-secondary text-center fw-bold my-5">No movies found in the database!</div>'
            ]);
        }
    }
